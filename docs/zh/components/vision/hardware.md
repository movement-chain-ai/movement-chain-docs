...
### 我们的选择：MediaPipe Pose

!!! success "项目决策"
    **MediaPipe Pose** 是我们的姿态估计方案。Google 持续维护，开箱即用，精度足够 MVP 验证。

| 选择 | 模型 | AP (COCO) | 状态 |
|------|------|-----------|------|
| ✅ **当前方案** | MediaPipe Pose | ~70%* | `pip install mediapipe` 即用，v0.10.26 (2024.12) |
| 🔄 备选 (如需更高精度) | RTMPose-m | 75.8% | 需配置 mmpose，暂不采用 |
| 🔄 备选 (研究级) | ViTPose++ | 81.1% | 需 GPU 服务器，暂不考虑 |

> *MediaPipe 使用 33 关键点 vs COCO 的 17 点，AP 不直接可比

!!! note "为什么不现在就用更高精度的模型？"
    1. **MVP 优先验证产品价值**，而非技术极致
    2. MediaPipe 精度对高尔夫挥杆分析**足够用** (33 关键点，30+ FPS)
    3. 如果用户反馈"精度不够"，再评估升级路径
    4. 过早优化是万恶之源
...
| 方案 | 模型 | 平台 | 特点 | 状态 |
|-----|------|-----|------|------|
| **MediaPipe** | BlazePose | 跨平台 | 易集成、Google 维护 | ✅ **当前方案** |
| **RTMPose + ONNX** | RTMPose-m | iOS/Android | 高精度、开源 | 备选 |
| **MoveNet + TFLite** | MoveNet | iOS/Android | 谷歌生态 | 备选 |
...
详细供应商信息请参见 [视觉方案竞品](../../business-plan/competitors/vision-based.md)
...
- [视觉方案竞品](../../business-plan/competitors/vision-based.md)
