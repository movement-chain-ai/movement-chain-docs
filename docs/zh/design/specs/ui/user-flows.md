...
flowchart TD
    ...
    C --> D{Tab 选择}
    D -->|练习计划| E[TrainingPlan]
    D -->|练习库| F[练习分类列表]

    E --> G{选择计划}
    G -->|查看详情| H[PlanDetail]
    G -->|开始训练| I[PlanExecutor - 传感器检查]
...
flowchart TD
    ...
    F --> G{选择类型}
    G -->|站姿引导| H[GuidedPractice - 站姿]
    G -->|慢动作引导| I[GuidedPractice - 慢动作]
    G -->|全部动作| J[combined 模式]
