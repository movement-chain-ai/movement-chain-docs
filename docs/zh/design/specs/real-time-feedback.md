...
这是两个**不同维度**的分类，可以交叉组合：

|  | 控制性 | 非控制性 |
|---|---|---|
| **发力驱动** | 慢速力量训练、平板支撑、康复激活 | 大重量爆发、格斗出拳 |
| **动作驱动** | 瑜伽保持、技术拆解 | 跳水、体操空翻 |
| **混合型** | 高尔夫慢挥练习 | 高尔夫全挥杆、网球发球 |

> 📝 **注**: "发力驱动/动作驱动"是 Movement Chain 提出的分类框架，用于判断是否需要 EMG 检测。详见 [战略愿景](../../business-plan/product-strategy/strategic-vision.md#emg-验证的战略意义)。
...
!!! tip "产品启示"
    - **控制性运动**：适合初学者、康复用户、技术纠正场景 → 强调实时反馈
    - **非控制性运动**：适合进阶用户、运动表现优化 → 强调事后分析 + 对比

这个分类直接映射到下面的三种反馈模式：

| 运动类型 | 对应反馈模式 |
|---------|-------------|
| 控制性（静态） | Mode 1: Setup Check |
| 控制性（动态慢速） | Mode 2: Slow Motion Training |
| 非控制性 | Mode 3: Full Speed Analysis |
