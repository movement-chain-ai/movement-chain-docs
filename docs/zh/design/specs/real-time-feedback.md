...
慢动作训练（Mode 2）的实时反馈本质上是**模拟真人教练的现场教学**。
...
!!! warning "产品定位"
    **Movement Chain 不只是"分析工具"，而是"AI 教练"**
    ...
    Mode 2（慢动作训练）是把产品从"分析工具"升级为"AI 教练"的关键功能。
...
慢动作训练（Mode 2）是把 Movement Chain 从"分析工具"升级为"AI 教练"的关键功能。

| 维度 | 只有全速分析 | 有慢动作训练 |
|-----|------------|------------|
| **角色** | 像"考官"：只告诉你对错 | 像"教练"：教你怎么做对 |
| **学习方式** | 被动接受评判 | 主动建立肌肉记忆 |
| **反馈时机** | 事后才知道错了 | 当下就能体会正确感觉 |
| **用户情感** | 反复出错 → 挫败 | 逐步掌握 → 成就感 |

**与全速分析的配合**：

| 用户阶段 | 推荐比例 | 原因 |
|---------|---------|------|
| 初学者 | 80% 慢动作 + 20% 全速 | 先建立正确模式 |
| 中级 | 50% 慢动作 + 50% 全速 | 巩固 + 检验迁移 |
| 高级 | 20% 慢动作 + 80% 全速 | 微调 + 表现优化 |

!!! tip "核心洞察"
    全速分析告诉你"哪里错了"，慢动作训练教你"怎么做对"。

    两者必须结合：慢动作训练建立模式 → 全速分析验证迁移。
