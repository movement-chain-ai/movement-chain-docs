...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Scenario 1: SETUP CHECK (站姿检查)              延迟要求: <300ms                  │
│  ─────────────────────────────────────────────────────────────────────────  │
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Scenario 2: SLOW MOTION TRAINING (慢动作训练)   延迟要求: <500ms                  │
│  ─────────────────────────────────────────────────────────────────────────  │
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Scenario 3: FULL SPEED ANALYSIS (全速分析)      延迟要求: <500ms (挥杆后)          │
│  ─────────────────────────────────────────────────────────────────────────  │
│  正常速度挥杆 → 挥杆后即时分析 → 综合反馈                                         │
│  "这一杆节奏不错，但核心发力可以更早"                                             │
│                                                                             │
...
在设计训练场景和反馈通道之前，需要理解一个关键的运动分类维度：**动作过程中能否被干预**。
...
反馈通道是指系统向用户传递信息的**形式**，与训练场景（场景 1/2/3）是正交的两个维度。
...
| 训练场景 | 语音反馈 | 视觉反馈 | 触觉反馈 |
|---------|---------|---------|---------|
| **Scenario 1: 站姿检查** | ✅ 主要 | ✅ 辅助 | ❌ 不适用 |
| **Scenario 2: 慢动作训练** | ✅ 主要 | ✅ 辅助 | ✅ 可选 |
| **Scenario 3: 全速分析** | ✅ 辅助 | ✅ 主要（事后回放） | ❌ 太快无法干预 |
...
| 阶段 | 内容 | 优先级 | 说明 |
|------|------|--------|------|
| **MVP** | Scenario 3: 全速分析 + 挥杆后反馈 | 🔴 必须 | MVP 核心功能 |
| **Post-MVP** | Scenario 1: 站姿检查 | 🟡 高 | 需要静态姿态检测 |
| **Post-MVP** | Scenario 2: 慢动作训练 | 🟡 高 | 实时处理复杂度高 |
| **Post-MVP** | 触觉振动反馈 | 🟡 中 | 需要 Apple Watch 联动 |
| **Post-MVP** | LLM 个性化反馈 | 🟢 可选 | 按用户特征定制 |

!!! info "为什么 MVP 只做 Scenario 3?"
