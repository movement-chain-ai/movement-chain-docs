...
> **文档目的**: 定义三段式评估模式的延迟要求、触发条件和实现策略
>
> **核心洞察**: 不同评估模式有不同的延迟要求和反馈时机
>
---
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Step 1: SETUP CHECK (站姿检查)                 延迟要求: <300ms                  │
│  ─────────────────────────────────────────────────────────────────────────  │
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Step 2: SLOW MOTION ANALYSIS (慢动作分析)      延迟要求: <500ms                  │
│  ─────────────────────────────────────────────────────────────────────────  │
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Step 3: FULL SPEED ANALYSIS (全速分析)         延迟要求: <500ms (挥杆后)          │
│  ─────────────────────────────────────────────────────────────────────────  │
│  正常速度挥杆 → 挥杆后即时分析 → 综合反馈                                         │
│  "这一杆节奏不错，但核心发力可以更早"                                             │
│                                                                             │
...
在设计分析模式和反馈通道之前，需要理解一个关键的运动分类维度：**动作过程中能否被干预**。
...
反馈通道是指系统向用户传递信息的**形式**，与分析模式（场景 1/2/3）是正交的两个维度。
...
| 分析模式 | 语音反馈 | 视觉反馈 | 触觉反馈 |
|---------|---------|---------|---------|
| **Step 1: 站姿检查** | ✅ 主要 | ✅ 辅助 | ❌ 不适用 |
| **Step 2: 慢动作分析** | ✅ 主要 | ✅ 辅助 | ✅ 可选 |
| **Step 3: 全速分析** | ✅ 辅助 | ✅ 主要（事后回放） | ❌ 太快无法干预 |

---

## 6. 延迟预算

**延迟预算(Latency Budget)**是指：从用户做动作到系统给出反馈，整个过程需要多长时间，每个环节各分配多少时间。
...
|-----|---------|---------|------|---------|
| Setup Check | <300ms | TTS 生成 | 用户静止等待，语音是主要反馈方式，TTS 占总延迟 60% | 预缓存常用语音 |
| Slow Motion | <500ms | Vision 帧率 | 需要追踪连续动作，30fps 每帧 33ms 会累积误差 | 提高到 60fps |
| Full Speed | <500ms (挥杆后) | 分析复杂度 | 挥杆后需综合多指标判断，规则越多耗时越长 | 规则引擎优先 |
...
| 阶段 | 内容 | 优先级 | 说明 |
|------|------|--------|------|
| **MVP** | Step 3: 全速分析 + 挥杆后反馈 | 🔴 必须 | MVP 核心功能 |
| **Post-MVP** | Step 1: 站姿检查 | 🟡 高 | 需要静态姿态检测 |
| **Post-MVP** | Step 2: 慢动作分析 | 🟡 高 | 实时处理复杂度高 |
| **Post-MVP** | 触觉振动反馈 | 🟡 中 | 需要 Apple Watch 联动 |
| **Post-MVP** | LLM 个性化反馈 | 🟢 可选 | 按用户特征定制 |

!!! info "为什么 MVP 只做 Step 3?"
