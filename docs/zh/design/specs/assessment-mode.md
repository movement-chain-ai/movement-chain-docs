...
| **过程中反馈** | 流程提示（"好的，站姿检查完成"） | 纠正指令（"脚再宽一点"） |
| **问题呈现** | 汇总在评估报告中 | 实时语音反馈 |
| **输出** | 完整评估报告 | 动作模式建立 |
| **用户体验** | 像体检 — 先检查，后出报告 | 像教练 — 边做边教 |

### 1.2 评估报告 vs 引导反馈

评估模式过程中**只有流程提示**，问题描述都**汇总在最终报告**中：

| 阶段 | 评估模式（过程中） | 评估模式（报告中） | 引导模式（实时） |
|------|------------------|------------------|----------------|
| 站姿 | "好的，站姿检查完成" | "站距偏窄" | "脚再宽一点" |
| 慢动作 | "好的，慢动作检查完成" | "肩膀转动幅度不够" | "继续转肩" |
| 全速 | "评估完成，正在生成报告..." | "核心发力时机偏晚" | "从核心启动" |

**设计理念**：

1. **不打扰用户** — 过程中只提示流程，不分散用户注意力
2. **完整诊断** — 所有问题汇总后一起呈现，便于理解全貌
3. **用户自主性** — 报告呈现问题（客观描述），用户决定改不改
