...
**产品启示**: MVP 支持单视角，Post-MVP 考虑双视角同步
...
!!! note "与 MVP 开发阶段的关系"
    本节描述的是**对比算法的演进路径**，不是 MVP 开发阶段 (Phase 1-4)。
    对比策略会随产品成熟度逐步升级：MVP → Post-MVP。

### 6.1 MVP: 规则引擎 + 个人最佳 (方法 B)
...
### 6.2 Post-MVP: 添加职业参考库 + 统计/ML 方法 (方法 A/C/D)
...
│                                                              │
│  方法 C/D: 统计/ML 方法 (当收集 100+ 用户挥杆后)             │
│  - 应用 Poze 方法 (统计参考)                                 │
│  - 或训练 SwingNet 风格的嵌入网络                            │
│  - 提供更细粒度的反馈                                        │
│                                                              │
│  参考数据: 职业库 (GolfDB) + 个人最佳 + 用户数据             │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```
...
- [术语表](../../prerequisites/foundations/biomechanics-glossary.md) - X-Factor, Kinetic Chain 等定义
