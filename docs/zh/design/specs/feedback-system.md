...
| 类型 | 说明 | 适用场景 |
|-----|------|---------|
| **骨骼叠加** | 在视频上叠加检测到的骨骼 | 实时预览、回放分析 |
| **角度标注** | 显示关节角度数值 | 评估报告、对比分析 |
...
| 场景 | 延迟要求 | 主要瓶颈 | 原因 | 优化策略 |
|-----|---------|---------|------|---------|
| **站姿引导** | <300ms | TTS 生成 | 用户静止等待，语音是主要反馈方式 | 预缓存常用语音 |
| **挥杆引导** | <500ms | Vision 帧率 | 需要追踪连续动作，30fps 每帧 33ms | 提高到 60fps |
| **评估报告** | 无实时要求 | 无 | 挥杆结束后分析，无延迟约束 | - |
| **触觉干预** | <50ms | 硬件响应 | 挥杆过程中干预，要求极低延迟 | 本地处理，无网络 |
...
|-----|---------|---------|---------|------|
| **评估模式** | ✅ 报告讲解 | ✅ 视频回放 + 标注 | ❌ | 事后 |
| **练习模式** | ✅ Drill 指导 | ✅ 动作演示 | ❌ | 练习中 |
| **引导模式** | ✅ 实时提示 | ✅ 实时叠加 | ✅ 可选 | 动作中 |
| **实战模式** | ⚠️ 简短 | ⚠️ 最小化 | ✅ 主要 | 动作中 |

### 4.2 评估模式反馈

评估模式的反馈主要是**事后报告**，无实时延迟要求：
