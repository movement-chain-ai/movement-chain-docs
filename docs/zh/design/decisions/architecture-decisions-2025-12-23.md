...
### 1.1 [六边形架构](../../reference/software-glossary.md#1-六边形架构-hexagonal-architecture) (Hexagonal Architecture) {#11-六边形架构-hexagonal-architecture}
...
> 📖 **备注**: [MSE (Mean Squared Error)](../../reference/ml-glossary.md#4-mse-均方误差) — 预测误差的平方均值，越小越好。
...
在 2025 最新实践调研中，[Rust](../../reference/software-glossary.md#2-rust) 频繁出现在高性能 ML 系统讨论中。
...
| `anthropic` / `openai` | Phase 2 [LLM](../../reference/ml-glossary.md#5-llm-大语言模型) 功能 | 自然语言反馈时 |
...
- ✅ 3 个 Sensor Hub: 手臂 (Arm)、核心 (Core)、腿部 (Leg)
- ✅ 每个单元: 1x ESP32 + 1x IMU (I2C) + 1x EMG (ADC)
- ✅ 同一部位传感器共享 ESP32 时钟 → 微秒级同步
- ✅ 每个单元都必须有 IMU → 可检测 Impact 事件做跨单元同步
...
#### ⚠️ [BLE 时间抖动问题](../../reference/engineering-glossary.md#11-ble-抖动-ble-jitter) (2025-12 研究验证)
...
- 🔑 **关键洞察**: 每个单元都必须有 IMU — Impact 振动传遍全身，每个 IMU 都能检测到同一事件作为 T=0 参考点
- ❌ 如果某个单元只有 EMG 没有 IMU → 无法检测 Impact → 无法与其他单元对齐时间
...
**结论**: ML 推理选 AWS，[边缘](../../reference/software-glossary.md#6-边缘计算-edge-computing) API 可选 Cloudflare
...
| **[全球边缘](../../reference/software-glossary.md#6-边缘计算-edge-computing)** | ✅ 最强 | ⚠️ 区域性 | CF ✅ (API 网关) |
| **冷启动** | ✅ 极快 | ⚠️ 较慢 | CF ✅ |
...
| [LLM 反馈](../../reference/ml-glossary.md#5-llm-大语言模型) | 规则引擎足够 MVP | Phase 2 |
| 云端部署 | 本地优先验证 | Phase 2 |
