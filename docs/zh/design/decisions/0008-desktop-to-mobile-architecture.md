...
```mermaid
flowchart TB
    subgraph DEV["开发环境 (Python)"]
        D1["Phase 1-3<br/>算法开发 & 验证"]
        D2["Rerun.io 可视化"]
        D3["Jupyter 实验"]
    end

    subgraph BRIDGE["桥接 (Phase 3.5)"]
        B1["Python → Swift<br/>算法移植"]
        B2["对比测试<br/>&lt;1% 差异"]
    end

    subgraph PROD["生产环境 (Swift)"]
        P1["Phase 4+<br/>iOS App"]
        P2["最终用户"]
    end

    subgraph SHARED["共享资产"]
        S1["MediaPipe<br/>.tflite 模型"]
        S2["规则参数<br/>JSON/YAML"]
    end
...
```mermaid
flowchart LR
    P3["Phase 3<br/>Python 验证完成"] --> P35["Phase 3.5<br/>Swift 算法包<br/>(无 UI)"]
    P35 --> P4["Phase 4<br/>完整 iOS App"]
