...
**角速度 ω** 和 **线速度 v** 的关系是：**v = ω × r**

| 部位 | 角速度 ω (°/s) | 旋转半径 r (m) | 线速度 v (m/s) |
|------|----------------|----------------|----------------|
| 骨盆 | 477 | ~0.15 | ~1.25 |
| 躯干 | 552 | ~0.30 | ~2.9 |
| 手臂 | 1200 | ~0.60 | ~12.6 |
| 杆头 | 2500 | ~1.50 | ~65.4 ≈ 146 mph |
...
- **I (转动惯量) 减小** → **ω (角速度) 必须增大**

所以：

- **角速度递增**：因为转动惯量在减小（质量减少）
- **线速度递增更快**：因为 v = ω × r，角速度增加的同时半径也在增加
...
| 角速度 ω | 5.2x (477 → 2500) |
| 半径 r | 10x (0.15 → 1.50) |
| **线速度 v = ω × r** | **52x** (1.25 → 65) |

**结论**：线速度的递增幅度远大于角速度（52 倍 vs 5 倍）。角速度递增是因为 I 减小，而线速度递增更快是因为同时 r 也在增大。
...
## 4.6 为什么杆头速度最大？— 运动链的物理学

上面我们知道了"杆头速度最大"的现象，但**为什么**会这样？这需要从物理学角度分析。
...
- [x] 杆头速度最大的原因：**继承上游角动量** + **I 小** + **r 大**
- [x] 角动量只来自**地面反作用力**，肌肉控制**传递时机**和**输入能量**
