...
!!! warning "MVP 测试的是管道集成，不是算法精度"
    - **初期**: 使用 Mock 数据验证管道逻辑（见 [§2 并行开发策略](#2-并行开发策略)）
...
> 📐 **详细规格**: 评估模式的完整定义见 [评估模式](../specs/modes/assessment-mode.md)
...
> 📐 **详细规格**: [数据管道与AI](./data-pipeline.md) | [数据处理与指标计算](./metrics-calculation.md)
...
> 📐 **详细规格**: [§7.2 诊断规则](#72-mvp-6-条诊断规则) | [评估模式](../specs/modes/assessment-mode.md)
...
| **Phase 2** | 时间对齐精度 | <10ms | [数据管道](./data-pipeline.md) |
...
| **Phase 4** | E2E 延迟 | <500ms | [评估模式](../specs/modes/assessment-mode.md) |
...
| 方向 | 文档 | 内容 | 依赖 |
|------|------|------|------|
| **阈值调整** | [用户信息和阈值调整](../specs/user-profile-thresholds.md) | 按性别/年龄/体型调整阈值 | Post-MVP |
| **EMG 扩展** | [传感器映射](./metrics-calculation.md) | 2→4→6 通道 EMG | 硬件验证 |
...
| **Mode 1/2** | [评估模式](../specs/modes/assessment-mode.md) | 站姿检查 + 慢动作训练 | Post-MVP |
