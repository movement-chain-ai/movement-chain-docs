...
## 3. 单传感器数据处理与指标计算 {#3-单传感器数据处理与指标计算}
...
!!! question "是否需要 %MVC 归一化？（待定）"
    **MVC 测量的困难**：

    - **用户不理解概念**: "用最大力量收缩肌肉" 普通用户不知道怎么做
    - **很难真正用到最大力量**: 人会本能保留一点力量，怕受伤
    - **需要专业指导**: 运动实验室里有专业人员指导，告诉你怎么发力
    - **需要固定姿势**: 比如测量股四头肌 MVC，需要坐在专用设备上，腿固定住
    - **每块肌肉都要单独测**: 多个 EMG 通道意味着要测多次

    **对于消费级产品可能不现实。**

    **如果不做 MVC 归一化**：

    - 用 SD 阈值法（基线 + N 倍标准差）做 Onset 检测
    - 不需要 MVC
    - 只输出时间点，不输出 "%MVC" 这种百分比指标

    **如果要做归一化，可以考虑的替代方案**：

    - **简化版校准**: "做一次你感觉用力的挥杆" 而不是 "最大力量"
    - **自动估算**: 用历史数据中最大的那次作为参考值
    - **根本不做归一化**: 只比较同一个用户不同次挥杆的相对变化
...
- [四种对比方法](../specs/four-comparison-methods.md): Pro vs Amateur 的生物力学差异
