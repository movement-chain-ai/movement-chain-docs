...
### 1.3 核心价值链详解 {#core-value-chain}

数据从用户挥杆到最终反馈，经过 7 个阶段的处理。每个阶段都有明确的功能边界和技术实现。

---

#### 1.3.1 Stage 1: 用户挥杆 (User Swings) {#stage-1}
...
#### 1.3.2 Stage 2: 数据采集层 (Data Collection) {#stage-2}
...
> 📐 详见: [实时反馈规格](../specs/real-time-feedback.md) — 三种反馈模式的详细设计
