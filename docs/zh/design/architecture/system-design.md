...
| # | 层级 | 功能 | 关键组件 | 价值 |
|---|------|------|----------|------|
| 1 | 数据采集层 | 三模态数据采集 | Vision + IMU + EMG, ESP32 Sensor Hub, Flutter App | WHAT + WHEN + WHY 完整信息 |
...
**交付物**: 实时数据流稳定传输到 iPhone Flutter App
...
| **产品发布** | Mobile (iOS/Android) | Flutter + ONNX Runtime | 用户体验、实时反馈、离线运行 |
...
| 方案 | 适用场景 | MVP 选择 |
|------|----------|----------|
| **Flutter + Dart** | UI、业务逻辑、网络请求 | ✅ 选择 |
| **Rust FFI** | 性能关键代码 (信号处理、IMU 融合) | Phase 2 考虑 |
| **React Native** | 已有 React 团队 | ❌ 不选 |
| **原生开发** | 需要最大控制权 | ❌ 不选 |

**2025 年趋势**:

- Flutter 性能持续优化 (60-120 FPS)
- Rust FFI 成为 Flutter 性能优化首选 (替代 C/C++)
- 混合方案: Flutter UI + Rust 计算密集型代码
