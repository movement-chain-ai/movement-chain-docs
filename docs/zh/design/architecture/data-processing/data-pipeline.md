...
| 数据如何变成用户反馈? | 特征提取 → Kinematic Prompt → LLM 反馈 (挥杆后) | [§2 数据流详解](#2-数据流详解) |
| "标准"从哪里来? | TPI + 学术论文，基于研究不是猜测 | [§3 标准与阈值来源](#3-标准与阈值来源) |
| 用户实际看到什么? | 简单界面 + 语音反馈，不是复杂图表 | [§2.5 D6-D7 用户体验层](#4-用户体验设计) |
...
### 2.1 D1-D2 信号采集与数字化 {#21-d1-d2-信号采集与数字化}
...
!!! warning "重要区分"
    ...
    用户体验设计见 [§2.5 D6-D7 用户体验设计](#4-用户体验设计)。
...
- [数据处理与指标计算](./metrics-calculation.md) - 数据计算公式
