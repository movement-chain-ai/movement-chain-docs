...
### 1.2 ONNX Runtime 延迟引入 ✅ 确认

**关键发现**: MediaPipe 自带 TFLite 推理引擎，ONNX Runtime 对于 MVP1 是**冗余的**
...
**关键研究**: CaddieSet (CVPR 2025) 验证了我们的架构选择

```text
┌─────────────────────────────────────────────────────────────────────────────┐
│                    CaddieSet 研究发现 RESEARCH VALIDATION                    │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   核心结论: 特征工程 + 简单模型 > 端到端深度学习                              │
│                                                                             │
│   球速预测 MSE 对比:                                                         │
│   ─────────────────────────────────────                                     │
│   Random Forest (Pose→特征→ML)     : 8.80   ← 最佳!                        │
│   XGBoost (Pose→特征→ML)           : 10.15                                 │
│   Vision Transformer (原始图像→DL) : 28.41  ← 3x 更差                       │
│   MobileNet V3 (原始图像→DL)       : 32.32  ← 4x 更差                       │
│                                                                             │
│   对我们架构的意义:                                                          │
│   ─────────────────────────────────────                                     │
│   ✅ MediaPipe → 特征提取 (X-Factor, Tempo) → 规则引擎 = 科学验证的正确路径   │
│   ✅ 不需要复杂的 Video Transformer                                         │
│   ✅ 高尔夫是生物力学约束运动 — 领域特征比像素更有效                          │
...
### 2.1 Python vs Rust 决策 ✅ 混合策略

**决策背景**:

在 2025 最新实践调研中，Rust 频繁出现在高性能 ML 系统讨论中。需要明确：是否应该用 Rust 重写核心逻辑？

**分析结论**: **Python + Rust-backed SDKs** 是最佳策略
...
│   ┌─────────────────────┐           ┌─────────────────────┐                │
│   │  LSM6DSV16X (IMU)   │           │  LSM6DSV16X (IMU)   │                │
│   │  MyoWare 2.0 (EMG)  │           │  MyoWare 2.0 (EMG)  │                │
│   │         │           │           │         │           │                │
│   │   同一时钟源 ────────┼───────────┼───同一时钟源        │                │
...
│   │ 跨 ESP32 (手臂↔核心)    │ <1 ms      │ 69-477 μs │ ✅    │            │
│   │ 跨设备 (ESP32↔手机)     │ <10 ms     │ <5 ms     │ ✅    │            │
│                                                                             │
