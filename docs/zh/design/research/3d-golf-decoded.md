...
|------|------|-----------------|-------------------|
| **认知阶段** | 理解动作要领，慢速、有意识执行 | ✅ 教学动画覆盖 | ⚠️ 未覆盖（机会点） |
| **联结阶段** | 减少错误，动作逐渐流畅 | ❌ 未覆盖 | ✅ 实时反馈辅助 |
| **自动化阶段** | 动作内化，全速无意识执行 | ❌ 未覆盖 | ✅ 持续训练监测 |

我们的产品重点在于联结和自动化阶段的辅助训练，但**认知阶段同样重要**——用户在练习时也需要理解"为什么要这样做"。我们在指导用户练习时提供**视觉和听觉反馈**，这些反馈也承担着提高用户认知的作用。因此，他们在认知阶段的界面设计值得我们借鉴：

| 借鉴维度 | 他们的做法 | 我们的应用场景 | 我们可以超越 |
|---------|-----------|---------------|-------------|
| **肌肉可视化** | 3D动画、肌肉高亮 | 训练实时反馈界面、动作回放 | 结合 **sEMG 实时数据**，显示真实肌肉激活 |
| **角度标注** | 角度圆、数值显示 | 实时数据叠加在动作视频上 | 用 **IMU 数据**提供更精确的实时角度 |
| **信息层次** | 半透明叠加、不遮挡主体 | 实时数据叠加在动作视频上 | 学习他们的半透明线条设计 |
| **对比展示** | 正确vs错误、多角度同步 | 用户动作vs标准动作对比 | 提供 **与职业选手/个人最佳** 的叠加对比 |
| **关键点强调** | 暂停标注、高亮发力肌群 | 实时提示需要纠正的部位 | 用 **语音反馈** 实时播报，不需要看屏幕 |
| **轨迹可视化** | 轨迹线、运动路径 | 让用户理解动作轨迹 | 学习他们的轨迹线设计 |
| **个性化** | 通用教学动画 | — | **个性化 Avatar** 基于用户体型生成 |
