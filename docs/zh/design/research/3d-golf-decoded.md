...
| 借鉴维度 | 他们的做法 | 我们可应用的场景 | 我们可以做的 |
|---------|-----------|-----------------|-------------|
| **肌肉可视化** | 3D动画、肌肉高亮 | 训练指导视频、视频回放 | 在指导视频中展示肌肉发力部位 |
| **角度标注** | 角度圆、数值显示 | 视频回放叠加数据 | 用 **IMU 数据**在回放中标注关节角度 |
| **信息层次** | 半透明叠加、不遮挡主体 | 视频回放叠加数据 | 学习他们的半透明线条设计 |
| **对比展示** | 正确vs错误、多角度同步 | 视频回放 | 用户动作 vs 标准动作对比 |
| **关键点强调** | 暂停标注、高亮发力肌群 | 训练指导视频 | 在关键动作点暂停并标注 |
| **轨迹可视化** | 轨迹线、运动路径 | 视频回放 | 叠加运动轨迹线 |
| **数据图表** | — | UI界面 | sEMG、IMU 数据的可视化图表 |
