...
我们的系统需要同时处理三种数据流：**视频 (MediaPipe)** + **IMU (100Hz)** + **EMG (200Hz)**。
...
| **验证 IMU 时间同步** | 100Hz 角速度曲线与视频帧对齐，一眼看出延迟 |
| **分析 EMG 肌肉激活** | 200Hz 信号与动作阶段对齐，验证"核心先启动"规则 |
...
│                                                                          │
│  🎯 高频时序数据                                                         │
│     支持 kHz 级别数据 (我们的 100Hz IMU、200Hz EMG 绑绑有余)             │
│                                                                          │
│  🎯 完全开源                                                             │
