...
| **Fatigue Threshold** 疲劳阈值 | <50% 初始激活 | 50-70% | 70-85% | >85% 初始激活 | EMG Research |
...
- **疲劳检测**: 当激活水平降至初始值的 <70% 时，表明肌肉疲劳，应休息或调整训练强度
...
| 优先级 | 名称 | 含义 | 反馈策略 | 示例规则 |
|--------|------|------|----------|----------|
| **P0** | 严重问题 | 必须修正，严重影响挥杆效果或有受伤风险 | 立即语音提醒，优先于其他反馈 | 倒序运动链、过度手臂挥杆 |
| **P1** | 重要改进 | 影响表现，但不紧急 | 挥杆后反馈，按重要性排序 | X-Factor 不足、节奏问题 |
| **P2** | 优化建议 | 锦上添花，进阶提升 | 可选反馈，留待详细分析 | 髋部滑移、重心偏移 |
| **P3** | 参考信息 | 数据展示，无明确建议 | 仅在报告中显示 | S-Factor、速度递增分析 |

**MVP 范围**: 初期只实现 P0 + P1 规则 (共 6 条)，P2-P3 留待后续版本迭代。

> 在 [MVP 开发计划](../../design/architecture/mvp-plan.md#72-mvp-6-条诊断规则) 中查看 MVP 具体选用的 6 条规则。
