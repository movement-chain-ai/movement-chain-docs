# 参考资料

> 学术资源与归档材料

## 术语表

| 文档 | 描述 |
|------|------|
| [高尔夫术语表](golf-glossary.md) | 高尔夫基础术语解释（差点、节奏比、杆头速度等） |
| [物理学术语表](physics-glossary.md) | 力学概念（角速度、角动量、转动惯量等） |
| [工程术语表](engineering-glossary.md) | 嵌入式系统/传感器术语（采样率、IMU、漂移、边缘 AI 等） |
| [软件架构术语表](software-glossary.md) | 软件设计模式（六边形架构、Clean Architecture 等） |
| [机器学习术语表](ml-glossary.md) | ML 术语（推理引擎、TFLite、模型部署等） |
| [商业术语表](business-glossary.md) | 商业术语（CAGR、BOM 等） |

## 学术资源

| 文档 | 描述 |
|------|------|
| [学术数据集](academic-datasets.md) | 公开学术数据集资源 (Fit3D, MM-Fit, FLAG3D 等) |
| [高尔夫生物力学研究](golf-research.md) | 高尔夫挥杆分析论文 (GolfDB, IMU 相位检测, DTW) |
| [开源工具与代码库](open-source-tools.md) | 开源项目参考 (MediaPipe, SwingNet, dtw-python) |

## 相关文档

- [ML 训练数据集](../development/ml-training/datasets.md) - 模型训练相关
- [系统设计](../design/architecture/system-design.md) - 技术架构
- [生物力学入门](../design/foundations/biomechanics-101.md) - 挥杆生物力学基础
